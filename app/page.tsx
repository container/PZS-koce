  ...
  return <main className={`${styles.app} ${tab === "map" ? styles.mapApp : ""}`}>
    ...
    {tab === "map" ? <section className={styles.mapView}>
      <aside className={styles.listPane}>
        ...
        {availabilityLoading && !availability && <div className={styles.listLoading}><span className={styles.spinner} aria-hidden="true" />{language === "sl" ? "Razpoložljivost se nalaga v ozadju …" : "Availability is loading in the background …"}</div>}
...
function DateRange({ language, arrival, departure, onChange }: { language: Language; arrival: string; departure: string; onChange: (arrival: string, departure: string) => void }) {
  const [open, setOpen] = useState(false); const [month, setMonth] = useState(() => new Date(`${arrival}T12:00:00`)); const [draftStart, setDraftStart] = useState<string | null>(null); const [draftEnd, setDraftEnd] = useState<string | null>(null);
  ...
  return <div className={styles.dateRange}><button className={styles.dateTrigger} type="button" onClick={openPicker} aria-expanded={open} aria-haspopup="dialog"><span><small>{language === "sl" ? "Prihod" : "Arrival"}</small>{format(arrival, language)}</span><span><small>{language === "sl" ? "Odhod" : "Departure"}</small>{format(departure, language)}</span></button>{open && <div className={styles.calendarOverlay} role="presentation" onMouseDown={close}><section className={styles.calendarPopover} role="dialog" aria-modal="true" aria-label={language === "sl" ? "Izberite datuma bivanja" : "Choose stay dates"} onMouseDown={(event) => event.stopPropagation()}><div className={styles.calendarTitle}><strong>{language === "sl" ? "Izberite datuma bivanja" : "Choose stay dates"}</strong><button type="button" className={styles.calendarClose} onClick={close} aria-label={language === "sl" ? "Zapri izbirnik datumov" : "Close date picker"}>×</button></div><button type="button" className={styles.monthNav} disabled={shownMonth <= firstAllowedMonth} onClick={() => setMonth((d) => new Date(d.getFullYear(), d.getMonth() - 1, 1))} aria-label={language === "sl" ? "Prejšnji mesec" : "Previous month"}>‹</button><Calendar language={language} month={month} minDate={minDate} maxDate={maxDate} selectedStart={draftStart ?? arrival} selectedEnd={draftEnd ?? undefined} onChoose={choose} /><div className={styles.secondCalendar}><Calendar language={language} month={new Date(month.getFullYear(), month.getMonth() + 1, 1)} minDate={minDate} maxDate={maxDate} selectedStart={draftStart ?? arrival} selectedEnd={draftEnd ?? undefined} onChoose={choose} /></div><button type="button" className={styles.monthNav} disabled={shownMonth >= lastAllowedMonth} onClick={() => setMonth((d) => new Date(d.getFullYear(), d.getMonth() + 1, 1))} aria-label={language === "sl" ? "Naslednji mesec" : "Next month"}>›</button><div className={styles.calendarActions}><p>{draftEnd ? `${format(draftStart!, language)} ${language === "sl" ? "do" : "to"} ${format(draftEnd, language)}` : language === "sl" ? "Izberite datum prihoda in odhoda" : "Choose an arrival and departure date"}</p><button type="button" onClick={close}>{language === "sl" ? "Prekliči" : "Cancel"}</button><button type="button" className={styles.applyDates} onClick={apply} disabled={!draftStart || !draftEnd}>{language === "sl" ? "Potrdi datuma" : "Apply dates"}</button></div></section></div>}</div>;
}

function Calendar({ language, month, minDate, maxDate, selectedStart, selectedEnd, onChoose }: { language: Language; month: Date; minDate: string; maxDate: string; selectedStart: string; selectedEnd?: string; onChoose: (date: string) => void }) { const first = new Date(month.getFullYear(), month.getMonth(), 1); const days = Array.from({ length: new Date(month.getFullYear(), month.getMonth() + 1, 0).getDate() }, (_, i) => new Date(month.getFullYear(), month.getMonth(), i + 1)); const weekdays = language === "sl" ? ["Po", "To", "Sr", "Če", "Pe", "So", "Ne"] : ["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"]; return <section className={styles.calendar}><strong>{month.toLocaleDateString(dateLocale(language), { month: "long", year: "numeric" })}</strong><div className={styles.weekdays}>{weekdays.map((day) => <span key={day} className={day === (language === "sl" ? "So" : "Sa") || day === (language === "sl" ? "Ne" : "Su") ? styles.weekendLabel : ""}>{day}</span>)}</div><div className={styles.calendarDays}>{Array.from({ length: (first.getDay() + 6) % 7 }).map((_, i) => <span key={`empty-${i}`} />)}{days.map((day) => { const value = iso(day); const selected = value === selectedStart || value === selectedEnd; const inRange = selectedEnd && value > selectedStart && value < selectedEnd; const weekend = day.getDay() === 0 || day.getDay() === 6; const disabled = value < minDate || value > maxDate; return <button key={value} disabled={disabled} className={`${weekend ? styles.weekendDay : ""} ${selected ? styles.selectedDay : ""} ${inRange ? styles.rangeDay : ""}`} onClick={() => onChoose(value)}>{day.getDate()}</button>; })}</div></section>; }

function HutCard({ hut, result, language, onClick }: { hut: Hut; result?: HutAvailabilitySummary; language: Language; onClick: () => void }) { const note = !result || result.status === "pending" ? "" : result.status === "error" ? language === "sl" ? "Preverjanje razpoložljivosti ni uspelo" : "Availability check failed" : result.availableCount ? language === "en" ? `${result.availableCount} rooms / beds available` : `Na voljo: ${result.availableCount} sob / ležišč` : language === "sl" ? "Ni razpoložljivih sob / ležišč" : "No rooms / beds available"; return <button className={styles.hutCard} onClick={onClick}>{hut.photoUrl && <Image src={hut.photoUrl} alt="" width={92} height={76} className={styles.hutPhoto} unoptimized />}<span><strong>{hut.name}</strong><small>{hut.region}</small>{note && <em className={result?.availableCount ? styles.availableNote : styles.unavailableNote}>{note}</em>}</span></button>; }
function HutDetail({ hut, result, language, onClose }: { hut: Hut; result?: HutAvailabilitySummary; language: Language; onClose: () => void }) { const units = result?.results ?? []; const closeRef = useRef<HTMLButtonElement>(null); useEffect(() => { const previousOverflow = document.body.style.overflow; document.body.style.overflow = "hidden"; closeRef.current?.focus(); const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape") onClose(); }; window.addEventListener("keydown", onKeyDown); return () => { document.body.style.overflow = previousOverflow; window.removeEventListener("keydown", onKeyDown); }; }, [onClose]); const summary = !result || result.status === "pending" ? language === "sl" ? "Preverjanje razpoložljivosti za izbrana datuma …" : "Checking availability for the selected dates …" : result.availableCount ? language === "sl" ? `Za izbrana datuma je na voljo ${result.availableCount} sob / ležišč.` : `${result.availableCount} rooms / beds available for the selected dates.` : language === "sl" ? "Za izbrana datuma ni razpoložljivih sob / ležišč." : "No rooms / beds available for the selected dates."; return <section className={styles.detailPanel} role="dialog" aria-modal="true" aria-labelledby="hut-detail-title"><div className={styles.sheetHandle} aria-hidden="true" /><button ref={closeRef} className={styles.closeButton} type="button" onClick={onClose} aria-label={language === "sl" ? "Zapri podrobnosti koče" : "Close hut details"}>×</button>{hut.photoUrl && <Image src={hut.photoUrl} alt="" width={560} height={300} className={styles.detailPhoto} unoptimized />}<p className={styles.detailRegion}>{hut.region}</p><h2 id="hut-detail-title">{hut.name}</h2><p>{summary}</p><h3>{language === "sl" ? "Razpoložljivost po enotah" : "Availability by unit"}</h3><div className={styles.unitTable}>{units.map((unit) => <UnitRow key={unit.bentralUnitId} unit={unit} language={language} />)}</div><div className={styles.detailActions}>{hut.mapzsUrl && <a href={hut.mapzsUrl} target="_blank" rel="noreferrer">{language === "sl" ? "Več o koči na maPZS" : "More about this hut on maPZS"}</a>}<a href={hut.pzsUrl} target="_blank" rel="noreferrer">{language === "sl" ? "Stran koče na PZS" : "Hut page on PZS"}</a></div></section>; }
...
function WeekView({ huts, language }: { huts: Hut[]; language: Language }) {
  ...
  const hutsByRegion = useMemo(() => {
    const groups = new Map<string, Hut[]>();
    for (const hut of huts) groups.set(hut.region, [...(groups.get(hut.region) ?? []), hut]);
    return [...groups.entries()].sort(([a], [b]) => a.localeCompare(b, "sl"));
  }, [huts]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await fetch("/api/huts/week-availability", { method: "POST", headers: { "Content-Type": "application/json" }, signal: controller.signal, body: JSON.stringify({ stays: days.map(({ arrivalDate, departureDate }) => ({ arrivalDate, departureDate })), hutIds: huts.map((hut) => hut.id) }) });
        if (!response.ok || controller.signal.aborted) return;
        const data = await response.json() as { availability?: Array<{ hutId: string; arrivalDate: string; status: "available" | "unavailable" | "unknown" }> };
        if (!Array.isArray(data.availability)) return;
        const next: Record<string, Record<string, "available" | "unavailable" | "unknown">> = {};
        for (const hut of huts) next[hut.id] = Object.fromEntries(days.map((day) => [day.arrivalDate, "unknown"]));
        for (const item of data.availability) if (next[item.hutId]) next[item.hutId][item.arrivalDate] = item.status;
        setGrid(next);
      } catch (error) {
        if (!(error instanceof DOMException && error.name === "AbortError")) console.error("Could not load weekly availability", error);
      }
    })();
    return () => controller.abort();
  }, [days, huts]);
  ...
  return <section className={styles.overview}><div className={styles.overviewTools}><div><button disabled={!canGoPrevious} onClick={() => setOffset((value) => Math.max(1, value - 7))}>{language === "sl" ? "Prejšnji teden" : "Previous week"}</button><button onClick={() => setOffset(1)}>{language === "sl" ? "Danes" : "Today"}</button><button disabled={!canGoNext} onClick={() => setOffset((value) => value + 7)}>{language === "sl" ? "Naslednji teden" : "Next week"}</button></div></div><section className={styles.weekBoard}><div className={styles.weekHeading}><strong>{language === "sl" ? "Razpoložljivost za 7 dni" : "7-day availability"}</strong><span>{language === "sl" ? "Razpoložljivost za eno noč po kočah" : "One-night availability by hut"}</span></div><div className={styles.weekLabels}><span>{language === "sl" ? "Koča" : "Hut"}</span>{days.map((day) => <span key={day.arrivalDate}>{day.label}</span>)}</div>{hutsByRegion.map(([region, regionHuts]) => <div key={region}><div className={styles.regionRow}><strong>{region}</strong></div>{regionHuts.map((hut) => <div className={styles.weekRow} key={hut.id}><strong>{hut.name}</strong>{days.map((day) => { const status = grid[hut.id]?.[day.arrivalDate] ?? "unknown"; return <span className={`${styles.dayCell} ${styles[status]}`} key={day.arrivalDate}>{statusLabel(status)}</span>; })}</div>)}</div>)}</section><section className={styles.weekCards} aria-label={language === "sl" ? "Razpoložljivost koč za 7 dni" : "7-day hut availability"}>{hutsByRegion.map(([region, regionHuts]) => <section key={region} className={styles.weekRegion}><h2>{region}</h2>{regionHuts.map((hut) => <article key={hut.id} className={styles.weekCard}><h3>{hut.name}</h3><div>{days.map((day) => { const status = grid[hut.id]?.[day.arrivalDate] ?? "unknown"; return <span className={`${styles.weekDay} ${styles[status]}`} key={day.arrivalDate}><small>{day.label}</small><b>{statusLabel(status)}</b></span>; })}</div></article>)}</section>)}</section></section>;
}
